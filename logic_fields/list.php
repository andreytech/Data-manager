...
include_once(JPATH_ADMINISTRATOR.DS."components".DS."com_datamanager".DS."fields".DS."field.php");
...
class ListField extends Field {
	...
	function get_field() {
		...
		if($this->read_only) {
			...
			foreach ($this->items as $val => $title) {
				$val = trim($val);
				$title = trim($title);
				$retval .= ' <option value="'.$val.'" '.($this->value==$val?' selected ':'').'>'.$title."</option>\n";
			}
			$retval .= '</select>'."\n";
			return $retval;
		}
	...
	function loadFieldParams() {
		...
		$this->field_params->addField(new TextareaField('param_items'),JText::_('List items'));
	}
